...
- Renders responsive views for both desktop and mobile form factors.
- **Medicines Catalog Page**: Connects to live REST endpoints (`/api/medicines`, `/api/medicines/search`, `/api/medicines/categories`).
- **Interactive Monograph Modal**: Displays complete provenance, FDA citations, and manufacturer storage limits.
- Displays unambiguous distinction between statistical ML confidence and rule-based deterministic alerts.
